...
class LivePlot(FigureCanvas):
    def __init__(self, parent=None, width=5, height=3, dpi=100):
        ...
        self.ax.plot([], [], color="blue", label="A leading (y > 0)")
        self.ax.plot([], [], color="red", label="B leading (y < 0)")
        self.ax.plot([], [], color="gray", label="Tie (y = 0)")
    ...
    def _add_segment(self, seg, color):
    ...
    def add_point(self, step, lead):
        ...
        pc = ["blue" if y > 0 else "red" if y < 0 else "gray" for y in self.ydata]

        self.scatter.set_offsets(pts)
        self.scatter.set_facecolors(pc)
        self.scatter.set_edgecolors(pc)
        ...
        if len(self.xdata) >= 2:
            ...
            if y0 == 0 and y1 == 0:
                self._add_segment([(x0, y0), (x1, y1)], "gray")

            elif y0 > 0 and y1 > 0:
                self._add_segment([(x0, y0), (x1, y1)], "blue")

            elif y0 < 0 and y1 < 0:
                self._add_segment([(x0, y0), (x1, y1)], "red")
                ...
                self._add_segment(seg1, c1)
                self._add_segment(seg2, c2)

        ymin, ymax = self.ax.get_ylim()
        if abs(lead) >= max(abs(ymin), abs(ymax)) - 1:
            new = max(5, abs(lead) * 1.3)
